...
class Bed extends Model
{
    ...
    public static function availableByType($bedTypeId, $exceptBedId = null)
    {
        return static::query()
            ->where('tenant_id', getLoggedInUser()->tenant_id)
            ->where('bed_type', $bedTypeId)
            ->where(function ($query) use ($exceptBedId) {
                $query->where('is_available', 1);
                if ($exceptBedId) {
                    $query->orWhere('id', $exceptBedId);
                }
            })
            ->where(function ($query) use ($exceptBedId) {
                $query->whereDoesntHave('bedAssigns', function ($assign) {
                    $assign->where('status', 1);
                });
                if ($exceptBedId) {
                    $query->orWhere('id', $exceptBedId);
                }
