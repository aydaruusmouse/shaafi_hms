        ...
        try {
            ...
            if ($input['patient_type'] == 2 && ! empty($input['patient_type'])) {
                ...
                if ($input['payment_type'] == 1) {
                    ...
                    $this->appointmentTransactionRepository->store($appointment);
                }
    ...
    public function getPatientDetails($email): JsonResponse
    {
        $patient = Patient::with('user')
            ...
            ->where(function ($query) use ($email) {
                $query->where('patient_unique_id', $email)
        ...
        if ($patient != null) {
            $data = [
                $patient->id => $patient->user->full_name.' ('.($patient->patient_unique_id ?: $patient->id).')',
            ];
        }
