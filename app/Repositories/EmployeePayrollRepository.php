...
class EmployeePayrollRepository extends BaseRepository
{
    ...
    {
        $input['basic_salary'] = $input['basic_salary'];
        $input['allowance'] = $input['allowance'];
        $input['deductions'] = $input['deductions'];
        $input['net_salary'] = $input['net_salary'];
        $input['month'] = DateTime::createFromFormat('!m', $input['month'])->format('F');
        $input['owner_type'] = EmployeePayroll::CLASS_TYPES[$input['type']];
        $employeePayroll = EmployeePayroll::create($input);
