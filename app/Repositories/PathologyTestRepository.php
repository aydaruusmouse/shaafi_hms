    ...
    public function store($input)
    {
        try {
            ...
            $pathologyTest = PathologyTest::create([
                ...
                'doctor_id' => $input['doctor_id'] ?? (getLoggedinDoctor() ? getLoggedInUser()->owner_id : null),
                'status' => (! empty(array_filter($input['patient_result'] ?? []))) ? 1 : ($input['status'] ?? 0),
            ]);
    ...
    public function update($input, $pathologyTest)
    {
        try {
            ...
            $pathologyTest->update([
                ...
                'status' => (! empty(array_filter($input['patient_result'] ?? []))) ? 1 : ($input['status'] ?? $pathologyTest->status),
            ]);
