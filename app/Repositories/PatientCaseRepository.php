...
class PatientCaseRepository extends BaseRepository
{
    ...
    public function getPatients()
    {
        ...
        $patients = Patient::with('patientUser')->get()->where('patientUser.tenant_id', getLoggedInUser()->tenant_id)->where('patientUser.status', '=', 1)->pluck('patientUser.full_name', 'id')->sort();
    ...
    {
        /** @var Doctor $doctors */
        $doctors = Doctor::with('doctorUser')->get()->where('doctorUser.tenant_id', getLoggedInUser()->tenant_id)->where('doctorUser.status', '=', 1)->pluck('doctorUser.full_name', 'id')->sort();
