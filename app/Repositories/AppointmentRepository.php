...
class AppointmentRepository extends BaseRepository
{
    ...
    public function getPatients()
    {
        ...
        $patients = Patient::with('patientUser')->get()->where(
            'patientUser.status',
            '=',
            1
        )->pluck('patientUser.full_name', 'id')->sort();
    ...
    public function sendAppointmentReminder()
    {
        ...
        foreach ($appointments as $appointment) {
            try {
                ...
                $input['patient_phone'] = isset($appointment->patient->user->region_code) ? $appointment->patient->user->region_code.$appointment->patient->user->phone : $appointment->patient->user->phone;
                ...
                if ($appointment->patient->user->phone && $appointment->patient->user->region_code) {
    ...
    {
        $data['bookingSlotArr'] = [];
        $bookingSlots = Appointment::whereDoctorId($inputs['doctor_id'])->whereDate(
            'opd_date',
            $inputs['editSelectedDate']
        )->get();
    ...
    {
        /** @var Doctor $doctors */
        $doctors = Doctor::whereId($id)->with('user')->get()->where('user.status', '=', 1)->pluck(
            'user.full_name',
            'id'
        );
    ...
    public function filter($status)
    {
        if ($status == 'all') {
            ...
            return Appointment::where('is_completed', Appointment::STATUS_PENDING)->where(
                'patient_id',
                getLoggedInUser()->owner_id
            )->with('patient', 'doctor', 'department')->orderBy('id', 'desc')->get();
        } elseif ($status == 'completed') {
            return Appointment::where('is_completed', Appointment::STATUS_COMPLETED)->where(
                'patient_id',
                getLoggedInUser()->owner_id
            )->with('patient', 'doctor', 'department')->orderBy('id', 'desc')->get();
        } elseif ($status == 'cancelled') {
            return Appointment::where('is_completed', Appointment::STATUS_CANCELLED)->where(
                'patient_id',
                getLoggedInUser()->owner_id
            )->with('patient', 'doctor', 'department')->orderBy('id', 'desc')->get();
        } elseif ($status == 'past') {
            return Appointment::whereDate('opd_date', '<', Carbon::today())->where(
                'patient_id',
                getLoggedInUser()->owner_id
            )->with('patient', 'doctor', 'department')->orderBy('id', 'desc')->get();
        } else {
    ...
    {
        $doctors = Doctor::where('doctor_department_id', $id)->where('tenant_id', getLoggedInUser()->tenant_id)->with('doctorUser')->orderBy(
            'id',
            'desc'
        )->get()->where(
            'doctorUser.status',
