...
class PrescriptionRepository extends BaseRepository
{
        ...
        try {
            ...
            if (isset($input['getMedicine'])) {
                $medicineBill = MedicineBill::create([
                    ...
                    'total' => 0,
                    'tax_amount' => 0,
                    'payment_type' => MedicineBill::MEDICINE_BILL_CASH,
                ]);

                foreach ($input['getMedicine'] as $key => $value) {
                    $medicine = Medicine::find($input['getMedicine'][$key]['medicine']);
                    $amount += $input['getMedicine'][$key]['day'] * $input['getMedicine'][$key]['dose_interval'] * $medicine->selling_price;
                    $qty = $input['getMedicine'][$key]['day'] * $input['getMedicine'][$key]['dose_interval'];
                    $saleMedicineArray = [
    ...
    {
        $settings = Setting::where('tenant_id', getLoggedInUser()->tenant_id)->pluck('value', 'key')->toArray();
